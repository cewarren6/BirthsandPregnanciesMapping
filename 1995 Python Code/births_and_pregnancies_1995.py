...
def findPopRast(popRastDir, iso3, grumpPopRast, countryBoundaries, outDir):
    ...
    # If no matches, clip grump population raster and resample to 100m resolution
        ...
        try:
            ...
            arcpy.CopyRaster_management(grumpPop100m, os.path.join(outDir, "pop1995%s.tif" % iso3))
            return os.path.join(outDir, "pop1995%s.tif" % iso3)
        finally:
...
def unajustedBirthsEstimates(ageFc, iso3, ageXls, isoNum, popRastPath, asfrPresent, unBirthsXls, outputGdb, outputDir):
    ...
    try:
        ...
        if asfrPresent == False:
            ...
            for row in range(1, ws.nrows):
                if ws.cell_value(row, 4) == iso3 and int(ws.cell_value(row, 2)) == 1995:
        ...
        arcpy.CopyRaster_management(births, os.path.join(outputDir, "%s1995unadjustedBirths.tif" % iso3), pixel_type="32_BIT_FLOAT")
...
def adminLevel2Estimates(adminBoundaryFc, iso3, urbanAreasShp, rastDir, outGdb):
    ...
    try:
        ...
        #for year, desc in itertools.product(("1995"), ("adjustedBirths", "pregnancies")):
...
def outputToExcel(outGdb, iso3, outXlsxDir):
    ...
    outputFc = os.path.join(outGdb, "birthsAndPregnancies%s" % iso3)
    # Fieldnames
    fields = ("ADM2_CODE", "ADM2_NAME", "urbanOrRural", "adjustedBirths1995", "pregnancies1995")
...
if __name__ == "__main__":
    ...
    urbanAsfrFc = "C:\\Google Drive\\BirthsandPregnanciesMapping\\asfr_1995.gdb\\asfrURBAN" # Urban ASFR data
    ruralAsfrFc = "C:\\Google Drive\\BirthsandPregnanciesMapping\\asfr_1995.gdb\\asfrRURAL" # Rural ASFR data
    dhsRegionsFc = "C:\\Google Drive\\BirthsandPregnanciesMapping\\dhsBoundaries\\1995\\dhsBoundaries.gdb\\subnational_boundaries" # DHS boundaries   
    ...
    popRastDir = "C:\\BirthsandPregnancies\\WorldPop1995" # Population raster directory
    grumpPopRast = "C:\\BirthsandPregnancies\\GRUMP\\grump_pop_1995.tif" # GRUMP 1995 population raster
...
# Output paths
    ...
    for country in countryList.findall("country"):
        ...
        adjustedBirthsEstimates(unBirthsXls, iso3, 1995, outDir)
